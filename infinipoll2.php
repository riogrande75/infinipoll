...
<?php
//Allg. Einstellungen
$moxa_ip = "udp://192.168.1.65"; //USR_TCP232_Server_UDP_Mode
//$moxa_ip = "192.168.1.60"; //USR_TCP232_Server_TCP_Mode

$moxa_port = 21109;	//Port des USR-TCP232
$moxa_timeout = 10;
...
write2file_string($tmp_dir."i2INFO.txt",$CMD_INFO);
...
//Ermitteln eines Anfangswertes fuer i2pv_ges:
$daybase = file_get_contents($tmp_dir."i2pv_ges_yday.txt");
if($daybase==0)
	{
...
while(true)
{
	...
	if($debug && $fp_log) @fclose($fp_log);
        ...
        if(date("H")=="23")
                ...
                $daybase_yday = file_get_contents($tmp_dir."i2pv_ges.txt");
                if($debug) logging("DAYBASE_YESTERDAY: ".$daybase_yday." KWh");
		write2file($tmp_dir."i2pv_ges_yday.txt",$daybase_yday);
                }
        // Wenn genau 1 Uhr Morgens, Totalcounter wird von i2pv_ges_yday als TagesBasis geholt:
        if(date("H")=="01")
                {
...
//	12.11.2018 QDVPR mitloggen
	fwrite($fp, "QDVPR".chr(0x0d)); //Query Device Volt Power Rated
	$byte=fgets($fp,19);
	$qdvpr=substr($byte,0,15);
//	echo "QDVPR:".$qdvpr."\n";
//	ENDE QDVPR
...
//	15.06.2019 Set GNMTx to 0
	fwrite($fp,"GNTMQ".chr(0x0d)); //QT Time inquiry 16byte
	$byte=fgets($fp,7);
	$erg=substr($byte,1,3);
	if($debug) logging("GNTMQ: ".$erg);
	if($erg=="01") {
		if($debug) logging("GNTMQ was 01, set it to 00");
		fwrite($fp,"GNTM0".chr(0x0d)); //Set GNMTQ to 01
		$byte=fgets($fp,7);
		$erg=substr($byte,1,3);
		if($debug) logging("GTNM0 was set, result ".$erg);
		}
        //Get day-counter
        $month=date("m");
        $year=date("Y");
	...
	if($debug) logging("DEBUG: QDVPR: $qdvpr"); // für Test Modbus 12.11.2018

        //schreibe akt. Daten in Files, die wiederum von 123solar drei Mal pro Sek. abgefragt werden:
	$ts = time();   //akt. Timestamp abfragen!
	write2file($tmp_dir."i2pv_ges.txt",$pv_ges);    //umwandlen in kWh!
    write2file($tmp_dir."i2ACV.txt",$ACV);
    write2file($tmp_dir."i2ACC.txt",$ACC);
    write2file($tmp_dir."i2ACF.txt",$ACF);
    write2file($tmp_dir."i2INTEMP.txt",$INTEMP);
    write2file($tmp_dir."i2DCINV.txt",$DCINV);
    write2file($tmp_dir."i2DCINC.txt",$DCINC);
    write2file($tmp_dir."i2INPA.txt",$INPA);
    write2file($tmp_dir."i2akt_power.txt",$akt_power);
	write2file($tmp_dir."i2BATTV.txt",$battvolt);
	write2file($tmp_dir."i2BATTCAP.txt",$battcap);
	write2file($tmp_dir."i2BATTCHAMP.txt",$battchamp);
	write2file_string($tmp_dir."i2STATE.txt",$modusT);
	write2file_string($tmp_dir."i2BATTSTAT.txt",$battstat);
	write2file_string($tmp_dir."i2BATTCODE.txt",$battcode);
	write2file_string($tmp_dir."i2ts.txt",date("Ymd-H:i:s",$ts));
